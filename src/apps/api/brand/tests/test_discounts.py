...
class DiscountSchema(json_schema.JsonSchema):
    ...
    balance = json_schema.IntegerProperty()
    hide = json_schema.BooleanProperty()
    enable = json_schema.BooleanProperty()


class BrandDiscountListApiTestCase(AuthenticatedBrandAPITestCase):
    def test_should_retrieve(self):
        brand = self.mixer.blend(models.Brand)
        self.authenticated(brand)

        self.mixer.blend(models.Discount, brand=brand)

        response = self.client.get(reverse("api:brand-discount-list"))
