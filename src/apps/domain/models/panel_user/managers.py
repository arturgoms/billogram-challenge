...
from apps.domain.models.panel_user import emails
...
class UserManager(BaseUserManager):
    ...
    def set_roles(self, instance, roles):  # noqa
        """
        Set the given collections of roles to panel_user.
        """
    ...
    def create_superuser(self, name, email, password=None, **kwargs):
        """
        Create an panel_user with superuser status.
    ...
    def invite_user(self, name, email, roles=None, **kwargs):
        """
        Create a new panel_user able to access administration panel.
        ...
        # generate panel_user password.
        password = self.make_random_password(length=8)

        with transaction.atomic():
            # create panel_user with password.
    ...
    def reset_password(self, instance):
        """
        Reset panel_user's password.

        Args:
            instance (domain.User, required): Affected panel_user instance.
        """
        # generate panel_user password.
        password = self.make_random_password(length=8)

        # update panel_user password.
        instance.set_password(password)
        instance.save()
    ...
    def promote_to_superuser(self, instance):  # noqa
        """
        Promote panel_user to superuser status.
