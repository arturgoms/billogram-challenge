...
from apps.admin.panel_user.forms.change_roles_form import ChangeRolesAdminForm
from apps.admin.panel_user.forms.invite_form import UserInviteAdminForm
...
@admin.register(models.PanelUser)
class PanelUserAdmin(PermissionsAdminMixin, SmartAdminMixin, admin.ModelAdmin):
        ...
        "change": [
            {
                ...
                and obj.status == models.PanelUser.Status.ACTIVE,
            ...
            },
            {
                ...
                == models.PanelUser.Status.BLOCKED,
            },
            {
                "title": _("Promote"),
    ...
    def active_field(self, obj):
        """
        Returns whether the panel_user is active.
        """
        return obj.status == models.PanelUser.Status.ACTIVE
    ...
    def invite_view(self, request):
        """
        Invite a new panel_user to administration panel.
        """
        ...
        if request.method == "POST" and form.is_valid():
            # invite panel_user.
            instance = models.PanelUser.objects.invite_user(
    ...
    def change_role_view(self, request, object_id):
        ...
        if request.method == "POST" and form.is_valid():
            # change role for panel_user.
            models.PanelUser.objects.set_roles(instance, form.cleaned_data["roles"])
    ...
    def block_view(self, request, object_id):
        """
        Chance the panel_user status to blocked.
        """
        ...
        if request.method == "POST":
            obj.status = models.PanelUser.Status.BLOCKED
            obj.save()
    ...
    def unblock_view(self, request, object_id):
        """
        Chance the panel_user status to active.
        """
        next_url = request.GET.get("next") or admin_url(
            "change", self.opts, args=[object_id]
        )
        obj = self.get_object(request, object_id)
        obj.status = models.PanelUser.Status.ACTIVE
        obj.save()
    ...
    def promote_view(self, request, object_id):
        """
        Promote panel_user to superuser
        """
        template_name = (
        ...
        if request.method == "POST":
            ...
            form = modelform_factory(models.PanelUser, fields=["is_superuser"])(
    ...
    def demote_view(self, request, object_id):
        """
        Demote panel_user to superuser
        """
        template_name = (
    ...
    def reset_password_view(self, request, object_id):
        ...
        if request.method == "POST":
            ...
            form = modelform_factory(models.PanelUser, fields=["password"])(
